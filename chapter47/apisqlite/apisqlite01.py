...
def movielookup(mykey, searchstring, typestring, yearstring):
    try:
        ## open URL to return 200 response
        if typestring != None and yearstring != None:
            resp = requests.get(f"{OMDBURL}apikey={mykey}&s={searchstring}&type={typestring}&y={yearstring}")
        elif typestring != None and yearstring == None:
            resp = requests.get(f"{OMDBURL}apikey={mykey}&s={searchstring}&type={typestring}")
        elif typestring == None and yearstring != None:
            resp = requests.get(f"{OMDBURL}apikey={mykey}&s={searchstring}&y={yearstring}")
        else:
    ...
    while True:
        ...
        while answer == "":
            ...
            ** Returned data will be written into the local database **
            ...
            2) Search for All Movies based on their type
            3) Search for All movies based on their release year
            4) Search for All Movies based on their type and release year
            99) Exit""")
        ...
        if answer == "1":
            ...
            searchstring = input("Search all movies in the OMDB. Enter search string: ")
            typestring = input("Enter a type to search: ")
            resp = movielookup(mykey, searchstring, typestring, "")["Search"]
            print(resp)
            if resp:
                # display the results
                print(resp)
            ...
            searchstring = input("Search all movies in the OMDB. Enter search string: ")
            yearstring = input("Enter a year to search: ")
            resp = movielookup(mykey, searchstring, "", yearstring)["Search"]
            print(resp)
            if resp:
                # display the results
                print(resp)
            ...
            typestring = input("Enter a type to search: ")
            yearstring = input("Enter a year to search: ")
            resp = movielookup(mykey, searchstring, typestring, yearstring)["Search"]
            print(resp)
            if resp:
                # display the results
                print(resp)
